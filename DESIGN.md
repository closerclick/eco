...
- **es** — Publicás un eco (texto + enlaces + tags) con tu ubicación. Quien esté
  en su radio de escucha lo descubre por `geo.closer.click`, lo recibe en su
...
- **en** — You post an eco (text + links + tags) with your location. Whoever has
  it in their listening radius discovers it via `geo.closer.click`, receives it
...
|-------|---------|------------|
| Identidad | `@closerclick/closer-click-identity` | firma cada eco; grafo de afinidad (contactos + ratings) |
| Transporte | `@closerclick/closer-click-proxy-client` | `sendByPubkey` para reply/repost/mención (cola offline 24 h) |
| Almacenamiento | `@closerclick/closer-click-store` | tu archivo local durable de ecos (lo único persistente) |
