...
async function main () {
  ...
  console.log('\n== GEO (geo.closer.click) ==')
  // 1) A publica un eco como beacon
  const eco = { id: 'e2e-' + Date.now(), author: A.pub, text: 'hola e2e', links: ['https://closer.click'], tags: [TAG], createdAt: Date.now() }
  ...
  console.log('\n== REPUTACIÓN (reputation.closer.click) ==')
  try {
    const r = await fetch('https://reputation.closer.click/health')
